...
#include "SAMR_headers\render.h"
#include "SAMR_headers\SamTabs.h"
...
void ResetSam() {
    // New code
    if (buffer != NULL) {
        // If the Init is called twice then 
        // wipe previous memory and continue
        free(buffer);
        input[256];
        ...
        phonemeIndexOutput[60];
        stressOutput[60];
        phonemeLengthOutput[60];
...
void Init() {
    ...
    // Give it a lot more than the other owner gave it because of crashes
	buffer = (char*)malloc(22050*22050);
